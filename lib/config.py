...
# PPTX 輸出設定
DEFAULT_SLIDE_WIDTH_INCHES = 13.333  # 16:9 橫式
DEFAULT_SLIDE_HEIGHT_INCHES = 7.5
PORTRAIT_SLIDE_WIDTH_INCHES = 7.5    # 直式（接近 A4 比例）
PORTRAIT_SLIDE_HEIGHT_INCHES = 10.0
DEFAULT_FONT_SIZE = 12

def get_slide_dimensions(page_width_pt: float, page_height_pt: float) -> tuple:
    """
    依據頁面原始尺寸決定投影片尺寸（英寸）。
    橫式（width >= height）→ 16:9；直式 → 接近 A4 直式比例。
    回傳 (slide_width_inches, slide_height_inches)
    """
    if page_width_pt >= page_height_pt:
        return DEFAULT_SLIDE_WIDTH_INCHES, DEFAULT_SLIDE_HEIGHT_INCHES
    else:
        return PORTRAIT_SLIDE_WIDTH_INCHES, PORTRAIT_SLIDE_HEIGHT_INCHES

def is_portrait(page_width_pt: float, page_height_pt: float) -> bool:
    """判斷頁面是否為直式"""
...
# 定義模型目錄
# 1. 優先查看是否有隨附在程式內部的模型 (唯讀)
BUNDLE_MODELS_DIR = get_resource_path("models")
# 2. 定義一個可寫入的使用者目錄 (用於存放下載的模型或快取)
USER_MODELS_DIR = os.path.join(os.path.expanduser("~"), ".notebooklm2pptx", "models")

if os.path.exists(BUNDLE_MODELS_DIR) and any(os.path.isfile(os.path.join(BUNDLE_MODELS_DIR, f)) for f in os.listdir(BUNDLE_MODELS_DIR) if f.endswith('.pth')):
    MODELS_DIR = BUNDLE_MODELS_DIR
else:
...
# EasyOCR 的 model_storage_directory 建議也指向這裡
...
DEFAULT_MASK_MIN_CONFIDENCE = 0.1
