    ...
    if model_name not in _lama_models:
        try:
            ...
            try:
                ...
                # If ModelManager fails for 'lama', try direct instantiation as fallback
                if model_name == 'lama':
                    print(f"  [Lama] ModelManager failed, trying direct import: {e}")
                    try:
                        from iopaint.model.lama import LaMa
                        # Create a dummy wrapper that mimics ModelManager's call signature
                        class LaMaWrapper:
                            def __init__(self):
                                # Attempt to instantiate LaMa directly. 
                                # Ensure device is passed as expected by InpaintModel.__init__
                                self.model = LaMa(device="cpu")
                                # Note: InpaintModel.__init__ typically calls init_model.
                                
                            def __call__(self, image, mask, config):
                                return self.model.forward(image, mask, config)
                                
                        _lama_models[model_name] = LaMaWrapper()
                        print(f"  [Lama] Direct import successful!")
                    except Exception as direct_e:
                        print(f"  [Lama] Direct import also failed: {direct_e}")
