...
def process_free(source_path, output_name=None, max_pages=None, ocr_engine='easyocr'):
    """
    使用免費 OCR 處理 PDF 或圖片資料夾。

    Args:
        source_path: PDF 檔案路徑，或包含圖片的資料夾路徑
        output_name: 輸出檔名（選填）
        max_pages: 最大處理頁數（選填，僅 PDF 有效）
        ocr_engine: 'tesseract' 或 'easyocr'
    """
    console.print(f"[bold cyan]使用免費 OCR 處理[/bold cyan]")
    console.print(f"OCR 引擎: {ocr_engine}")

    if not os.path.exists(source_path):
        console.print("[bold red]路徑不存在[/bold red]")
        return False

    # 1. 載入來源
    is_pdf = os.path.isfile(source_path) and source_path.lower().endswith('.pdf')

    console.print("\n[yellow]步驟 1/3: 載入來源...[/yellow]")
    try:
        if is_pdf:
            all_images, all_page_sizes = pdf_to_images(source_path)
            total_pages = len(all_images)

            if max_pages and max_pages < total_pages:
                images = all_images[:max_pages]
                page_sizes = all_page_sizes[:max_pages]
                console.print(f"[yellow]僅處理前 {max_pages} 頁（共 {total_pages} 頁）[/yellow]")
            else:
                images = all_images
                page_sizes = all_page_sizes
        else:
            image_paths = collect_images_from_paths([source_path])
            if not image_paths:
                exts = ", ".join(SUPPORTED_EXTENSIONS)
                console.print(f"[red]未找到支援的圖片（{exts}）[/red]")
                return False
            images, page_sizes = images_to_slides(image_paths)

        console.print(f"[green]✓ 載入完成，共 {len(images)} 頁[/green]")
    ...
    for i, img in enumerate(images):
        ...
        try:
            ...
            slides_data.append({'text_blocks': text_blocks, 'page_size': page_sizes[i]})
            console.print(f" [green]✓ {len(text_blocks)} 個區塊[/green]")

        except Exception as e:
            console.print(f" [red]✗ 失敗: {e}[/red]")
            slides_data.append({'text_blocks': [], 'page_size': page_sizes[i]})
    ...
    if not output_name:
        ...
        if is_pdf and max_pages and max_pages < len(all_images):
    ...
    try:
        ...
        for p in saved:
            console.print(f"\n[bold green]✓ 完成！{os.path.basename(p)}[/bold green]")
            console.print(f"  輸出位置: {os.path.abspath(p)}")
        return True
    except Exception as e:
        console.print(f"[red]✗ 失敗: {e}[/red]")
        return False
...
def process_pdf_free(pdf_path, output_name=None, max_pages=None, ocr_engine='easyocr'):
    return process_free(pdf_path, output_name, max_pages, ocr_engine)


if __name__ == "__main__":
    if len(sys.argv) < 2:
        console.print("[yellow]使用方式:[/yellow]")
        console.print('  python convert_free.py "PDF路徑或圖片資料夾"')
    ...
    success = process_free(source_path, max_pages=max_pages, ocr_engine=ocr_engine)
